...
namespace MyNETCoreLib
{
    ...
    public partial class AvlTree<T> : IList<T>, IList where T : IComparable<T>
    {
        ...
        public virtual bool Delete(T target)
        {
            ...
            if (target != null)
            {
                Root = DeleteNode(Root, target, ref rtn);
        ...
        public void RemoveAt(int index)
        {
            ...
            DeleteNode(node, node.Value, ref temp);
        }
        ...
        protected static Node<T> DeleteNode(Node<T> current, T target, ref bool found)
        {
            Node<T> parent;
            ...
            if (current != null)
            {
                ...
                {
                    current.Left = DeleteNode(current.Left, target, ref found);
                ...
                {
                    current.Right = DeleteNode(current.Right, target, ref found);
                ...
                else
                {
                    if (current.Right != null)
                    {
                        ...
                        current.Right = DeleteNode(current.Right, parent.Value, ref found);

                        if (Balance_Factor(current) == 2)//re-balancing
                        {
                            // (BW) Changed to using a ternary operator
                            current = Balance_Factor(current.Left) >= 0 ? RotateLL(current) : RotateLR(current);
                        }
        ...
        private static Node<T> AddRecursive(Node<T> current, Node<T> node, ref bool added)
        {
            ...
            {
                current.Left = AddRecursive(current.Left, node, ref added);
                // (BW) Moved Balance_Tree()
            }
            else if (node.Value.CompareTo(current.Value) > 0)
            {
                current.Right = AddRecursive(current.Right, node, ref added);
                // (BW) Moved Balance_Tree()
            }
